...
- `cloudfunctions/api/config.json`：云函数开放能力权限，包含内容安全接口
...
- 发布动态、评论、下班约局、申诉会先走微信内容安全审核
- 鱼塘上传图片会在云函数里下载后走图片安全审核
...
4. 右键 `cloudfunctions/api`，选择“上传并部署：云端安装依赖”。确认 `config.json` 一起上传。
...
本地缓存仍作为兜底使用。云函数失败时页面不会白屏，会退回本地缓存空态或本地数据。
